...
///////////////////////////////////////////////////////////////////
// 根據篩選條件顯示房間
function filterAndDisplayRooms(allRoom, minprice, maxprice, zone) {
    for (let index = 0; index < allRoom.length; index++) {
        const room = allRoom[index];
        // 添加篩選條件，價格在 minprice 和 maxprice 之間的才顯示
        //minprice & maxprice & zone都有值
        if (
            (!minprice || parseInt(room.money) >= minprice) &&
            (!maxprice || parseInt(room.money) <= maxprice) &&
            room.zone == zone
        ) {
            MakeRoomCard(index, room);
        }
        //minprice & maxprice 有值
        else if (
            (!minprice || parseInt(room.money) >= minprice) &&
            (!maxprice || parseInt(room.money) <= maxprice) &&
            zone == ""
        ...
        //zone有值
        else if (minprice == "" && maxprice == "" && room.zone == zone) {
            MakeRoomCard(index, room);
        }
        //minprice & maxprice & zone皆沒有值，直接回到首頁顯示全部資料
        else if (minprice == "" && maxprice == "" && zone == "") {
            MakeRoomCard(index, room);
        }
    }
}
...
function MakeRoomCard(index, room) {
    ...
    if (room.img_list && room.img_list.length > 0) {
        ...
        <div class="carousel-item ${i === 0 ? "active" : ""}">
          <img src="${img}" class="d-block w-100" style="max-height: 250px;" alt="Image ${
                i + 1
            }">
        </div>
      `;
  ...
  `;
    ...
    let finalcard = $("<div></div>")
        .addClass("col-md-12 col-lg-4 py-5")
        .attr("id", "showcards");
